...
pizzaJson.map((item, index) => {
    ...
    pizzaItem.querySelector("a").addEventListener("click", (e)=>{
        ...
        cs(".pizzaInfo--size").forEach((el, elIndex)=>{
            if(elIndex == 2) el.classList.add("selected");
            el.querySelector("span").innerHTML = pizzaJson[key].sizes[elIndex]
        });
    ...
    if (cart.length > 0) {
        ...
        let total = 0;
         cart.map((item, index, cart)=>{
             ...
             cartItem.querySelector(".cart--item-nome").innerHTML = `${pizzaItem.name} ${pizzaItem.sizes[item.size]}`;
